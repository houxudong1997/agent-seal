...
**agent-seal v1.0** is the first production-grade release. It transforms the v0.1 proof-of-concept into a deployable, scalable, and observable enterprise audit trail system for AI agents — ready for EU AI Act Article 12 compliance.
...
| **`AGENT_SEAL_DB_URL` config** | Standardized database URL across all backends. Replaces old `AGENT_SEAL_DIR` / `AGENT_SEAL_URI`. |
...
| **Structured logging** | `AGENT_SEAL_LOG_FORMAT=json` for ELK/Loki integration. Text format for human readability. |
...
| **Grafana dashboard** | Pre-built dashboard at `deploy/grafana/dashboards/agent-seal-overview.json`. 12 panels: event throughput, token usage, latency P50/P95/P99, error rate, and more. |

### Tracing & Observability

| Feature | Description |
|---|---|
| **LLM auto-tracing** | `import agent_seal.tracing.auto` — monkey-patches OpenAI/Anthropic SDKs. All LLM calls automatically recorded with tokens, latency, and cost. |
| **PII redaction** | `AGENT_SEAL_TRACE_PII_REDACT=1` — automatically strips PII from traced content. Uses regex patterns for emails, phones, SSNs, credit cards. |
| **Cost tracking** | Token → USD cost calculation. Per-model pricing. Read from `AGENT_SEAL_TRACE_COST_MODEL`. |
...
| **Digital signatures** | `Ed25519` signing. Each event optionally signed with a private key. `AGENT_SEAL_SIGNING_KEY` points to PEM file. |
...
| Feature | Description |
|---|---|
| **Slack webhooks** | `AGENT_SEAL_SLACK_WEBHOOK` — audit alerts delivered to Slack. Policy blocks, integrity failures, error spikes. |
| **Email alerts** | `AGENT_SEAL_SMTP_HOST` — SMTP-based email notifications. Configurable failure alerts. |
| **Failure notifications** | `AGENT_SEAL_NOTIFY_ON_FAILURE=1` — auto-notify on integrity breaks. |
...
| **Multi-stage Docker build** | Builder stage → production stage. No build tools in runtime image. Non-root user (`agentseal`). Health check built in. |
...
All environment variables now use the `AGENT_SEAL_` prefix:

| v0.1 | v1.0 | Notes |
|---|---|---|
| `AUDIT_DIR` | `AGENT_SEAL_AUDIT_DIR` | Still supported for backward compat |
| `DB_URL` / `DATABASE_URL` | `AGENT_SEAL_DB_URL` | Legacy vars still read as fallback |
| `API_KEYS` | `AGENT_SEAL_API_KEYS` | Legacy var still read as fallback |
| `SECRET_KEY` | `AGENT_SEAL_SECRET_KEY` | Legacy var still read as fallback |
| `SIGNING_KEY` | `AGENT_SEAL_SIGNING_KEY` | New namespace only |
| `LOG_LEVEL` | `AGENT_SEAL_LOG_LEVEL` | Legacy var still read as fallback |
...
#    AUDIT_DIR=./audit_logs  →  AGENT_SEAL_AUDIT_DIR=./audit_logs
...
3. **Large event payloads**: `input_snapshot` and `output_snapshot` are truncated to 8000 characters. Set `AGENT_SEAL_TRACE_MAX_LEN` to adjust (max 8000).
...
*agent-seal v1.0.0 — Tamper-evident. Production-ready. EU AI Act compliant.*
