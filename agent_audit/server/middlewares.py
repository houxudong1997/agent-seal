"""
agent-audit server middleware stack — CORS, GZip, API Key auth.
...
def setup_cors(app: FastAPI) -> None:
    """Configure CORS from AGENT_AUDIT_CORS_ORIGINS.
...
class APIKeyAuthMiddleware(BaseHTTPMiddleware):
    """Authenticate /api/* routes when ``AGENT_AUDIT_API_KEYS`` is configured.
...
def setup_api_key_auth(app: FastAPI) -> None:
    """Apply API-key auth middleware when AGENT_AUDIT_API_KEYS is set."""
