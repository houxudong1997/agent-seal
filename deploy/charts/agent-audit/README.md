# agent-audit Helm Chart

> **Tamper-evident audit trail for AI agents — production-grade Kubernetes deployment.**

This Helm chart deploys agent-audit on Kubernetes with PostgreSQL, Redis, NGINX reverse proxy, and optional production features (HPA, PDB, NetworkPolicy, ServiceMonitor).
...
helm install agent-audit ./deploy/charts/agent-audit \
...
```bash
kubectl rollout status deployment/agent-audit-api
```
...
kubectl port-forward svc/agent-audit-nginx 8080:80
...
```bash
helm install agent-audit ./deploy/charts/agent-audit \
  -f ./deploy/charts/agent-audit/values-prod.yaml \
  --set config.secretKey=$(openssl rand -hex 32) \
  --set config.apiKeys[0]="sk-***" \
  --set externalDb.url="postgresql://audit:***@rds.example.com:5432/agent_audit" \
...
| `ingress.hosts` | `[agent-audit.local]` | Host rules |
...
helm install agent-audit ./deploy/charts/agent-audit \
  ...
  --set externalDb.url="postgresql://audit:***@host:5432/agent_audit" \
...
```bash
helm upgrade agent-audit ./deploy/charts/agent-audit \
  -f ./deploy/charts/agent-audit/values-prod.yaml \
  --reuse-values
```
...
```bash
kubectl delete pvc -l app.kubernetes.io/instance=agent-audit
```
...
```bash
kubectl describe pod -l app.kubernetes.io/instance=agent-audit
```
...
```bash
kubectl logs statefulset/agent-audit-postgresql
```
...
```bash
kubectl logs deployment/agent-audit-api
```
...
```bash
kubectl describe hpa agent-audit-api
```
...
```bash
kubectl describe certificate agent-audit-tls
```
...
| `AGENT_AUDIT_DB_URL` | `externalDb.url` (or auto-generated if `postgresql.enabled=true`) |
| `AGENT_AUDIT_REDIS_URI` | `externalRedis.uri` (or auto-generated if `redis.enabled=true`) |
| `AGENT_AUDIT_SECRET_KEY` | `config.secretKey` |
| `AGENT_AUDIT_API_KEYS` | `config.apiKeys` (array) |
| All other `AGENT_AUDIT_*` vars | `config.*` (mapped 1:1) |
...
- **Issues**: https://github.com/agent-audit/agent-audit/issues
