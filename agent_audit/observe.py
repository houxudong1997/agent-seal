"""
@observe decorator — lightweight function tracing for agent-audit.

Inspired by LangFuse's @observe pattern, this decorator records every
decorated function call into the agent-audit event store, capturing:
  ...
  - auto-writes to agent-audit event storage via ``get_engine()``
...
class observe:
    """Decorator that traces function calls into the agent-audit trail.
