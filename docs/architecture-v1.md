...
agent-seal 是一个面向 AI Agent 的防篡改审计追踪系统。核心价值：
...
**理由**：agent-seal 的核心是**时间序列审计事件**，TimescaleDB 的超表（Hypertable）天然适合按时间分区的审计日志。写入性能满足企业初期需求，且未来可平滑扩展。
...
app = FastAPI(title="agent-seal API", version="1.0.0")
security = HTTPBearer()
...
| **可观测性** | 仅 agent-seal | agent-seal + 日志 | 全栈 (链路追踪 + 指标 + 日志) |
...
```python
# agent_seal/tracing/auto.py
# 一行启用
import agent_seal.tracing.auto  # auto-instrument OpenAI & Anthropic

# 之后所有 LLM 调用自动记录到 audit trail + llm_calls 表
```

```python
# agent_seal/tracing/openai_instrumentor.py
...
class OpenAITraceInstrumentor:
    ...
    def __init__(self, engine: "AuditEngine", tracer=None):
        self.engine = engine
        self.tracer = tracer or trace.get_tracer("agent-seal")
...
```python
# agent_seal/tracing/opentelemetry.py
# 使用标准 OTel 体系
...
# 4. agent-seal 同时从 trace 中提取审计事件
...
```python
# 用户在项目入口处一行启用：
import agent_seal.tracing  # 自动 patch OpenAI & Anthropic
...
services:
  ...
  nginx:
    ...
    volumes:
      ...
      - ./agent_seal/server/static:/usr/share/nginx/html:ro
    depends_on: [api]
  ...
  api:
    build:
      ...
      target: production
    environment:
      - AGENT_SEAL_DB_URL=postgresql://audit:${DB_PASSWORD}@db:5432/agent_seal
      - AGENT_SEAL_REDIS_URI=redis://redis:6379/0
      - AGENT_SEAL_SECRET_KEY=${SECRET_KEY}
      - AGENT_SEAL_SIGNING_KEY=/run/secrets/signing_key
      - AGENT_SEAL_AUTO_TRACE=${AUTO_TRACE:-1}
    secrets:
      - signing_key
    depends_on: [db, redis]
    restart: unless-stopped
  ...
  backup:
    ...
    entrypoint: ["/bin/sh", "-c"]
    command: |
      "0 3 * * * pg_dump -h db -U audit agent_seal | gzip > /backups/agent-seal-$$(date +\\%Y\\%m\\%d).sql.gz"
    environment:
...
```bash
# ── 核心配置 ──
AGENT_SEAL_DB_URL=postgresql://audit:PASSWORD@db:5432/agent_seal
AGENT_SEAL_REDIS_URI=redis://redis:6379/0
AGENT_SEAL_SECRET_KEY=<随机64字节hex>

# ── 存储 ──
AGENT_SEAL_STORAGE_BACKEND=postgresql    # jsonl|sqlite|postgresql
AGENT_SEAL_EVIDENCE_STORE=s3://bucket    # 证据包存储 (可选)

# ── 加密 & 签名 ──
AGENT_SEAL_SIGNING_KEY=/etc/secrets/ed25519.pem
AGENT_SEAL_ENCRYPTION_KEY=<32字节hex>    # AES-256

# ── LLM 追踪 ──
AGENT_SEAL_AUTO_TRACE=1                  # 启用自动追踪
AGENT_SEAL_TRACE_PII_REDACT=1            # 脱敏 LLM 请求
AGENT_SEAL_TRACE_MAX_LEN=4000            # 截断长度
AGENT_SEAL_TRACE_COST_MODEL=openai       # 成本计算模型

# ── API ──
AGENT_SEAL_API_KEYS=key1,key2            # 可选 API keys
AGENT_SEAL_CORS_ORIGINS=*                # CORS

# ── 通知 ──
AGENT_SEAL_SLACK_WEBHOOK=https://hooks.slack.com/...
AGENT_SEAL_SMTP_HOST=smtp.example.com
AGENT_SEAL_NOTIFY_ON_FAILURE=1

# ── 日志 ──
AGENT_SEAL_LOG_LEVEL=info
AGENT_SEAL_LOG_FORMAT=json               # json|text
```
