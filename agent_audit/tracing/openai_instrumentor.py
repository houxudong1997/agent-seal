...
``llm_calls`` table (SQLAlchemy), and pushed to the agent-audit hash-chain.
"""
...
logger = logging.getLogger("agent_audit.tracing.openai")
...
def _persist_llm_call(
    *,
    ...
    try:
        ...
        if not db_url:
            logger.debug("AGENT_AUDIT_DB_URL not set — skipping llm_calls write")
            return
...
def _resolve_tracer(tracer: Any = None) -> Any:
    ...
    try:
        ...
        return otel_trace.get_tracer("agent-audit")
    except ImportError:
