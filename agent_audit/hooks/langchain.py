"""
LangChain / LangChain-compatible CallbackHandler for agent-audit.

Records every LLM call, tool invocation, chain step, and agent decision
into the agent-audit hash-chained audit trail via the AuditEngine singleton.
...
from agent_audit.server.dependencies import get_engine
...
class AuditCallbackHandler(BaseCallbackHandler):  # type: ignore[valid-type,misc]
    """LangChain callback handler that records all activity into agent-audit.
