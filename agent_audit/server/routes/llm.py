...
async def llm_status():
    ...
    return {
        ...
        "env_config": {
            "AGENT_AUDIT_AUTO_TRACE": config.auto_trace,
            "AGENT_AUDIT_TRACE_PII_REDACT": config.trace_pii_redact,
            "AGENT_AUDIT_TRACE_MAX_LEN": config.trace_max_len,
            "AGENT_AUDIT_TRACE_COST_MODEL": config.trace_cost_model,
        },
    }
...
async def llm_disable():
    ...
    The env var AGENT_AUDIT_AUTO_TRACE must be set to 0 and the
    ...
    return {
        ...
        "message": (
            ...
            "Set AGENT_AUDIT_AUTO_TRACE=0 in .env and restart the server."
        ),
    }
...
async def llm_log(
    ...
    into agent-audit for traceability and cost tracking.
    """
