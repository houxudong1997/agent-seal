"""
agent-audit CLI — tamper-evident audit trail for AI agents.
...
def cmd_log():
    """Record a test event."""
    if len(sys.argv) < 4:
        print("Usage: agent-audit log <event_type> <output>")
        sys.exit(1)
...
def cmd_prompt():
    ...
    if len(sys.argv) < 4:
        print("Usage: agent-audit prompt <save|history|diff> ...")
        sys.exit(1)
    ...
    if sub == "save":
        if len(sys.argv) < 7:
            print("Usage: agent-audit prompt save <agent_id> <changed_by> <reason>")
        ...
        if len(sys.argv) < 7:
            print("Usage: agent-audit prompt diff <agent_id> <v1> <v2>")
            sys.exit(1)
        agent_id = sys.argv[4]
...
def cmd_serve():
    ...
    print(f"agent-audit Dashboard → http://{host}:{port}")
...
def main():
    if len(sys.argv) < 2 or sys.argv[1] not in COMMANDS:
        print("agent-audit — tamper-evident audit trail for AI agents")
        print(f"Commands: {', '.join(COMMANDS)}")
        print()
        print("Quick start:")
        print("  agent-audit verify           # Check integrity")
        print("  agent-audit trail            # View recent events")
        print("  agent-audit report <agent>    # EU AI Act compliance report")
        print("  agent-audit log <type> <msg>  # Record a test event")
        print("  agent-audit prompt save ...   # Version control prompts")
        sys.exit(1)
    COMMANDS[sys.argv[1]]()
